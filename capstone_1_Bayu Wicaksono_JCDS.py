...
def CreateNewPasien():
    while True: #berguna untuk menjalankan looping dari CreateNewPasien dengan menggunakan input angka untuk menjalankan fungsi conditional statement
        ...
        if(a == '1'):
            print_PasienTabulate(listPasien)  # Menampilkan data pasien sebelum menambahkan pasien baru
            ...
            print_PasienTabulate([temporary_table])  # Menampilkan data baru sebelum konfirmasi
...
def CariPasien(pasienLama):  
    for i in range(len(listPasien)): #bertujuan untuk mencari indeks(key) dari tiap pasien dalam listpasien berdasarkan Nomor Rekam Medis
